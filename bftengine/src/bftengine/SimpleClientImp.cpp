...
int SimpleClientImp::sendRequest(uint8_t flags,
  ...
  LOG_INFO(logger_, "Span context:" << span_context);
  ...
  if (isPreProcessRequired)
    ...
    reqMsg = new ClientRequestMsg(clientId_, flags, reqSeqNum, lengthOfRequest, request, timeoutMilli, msgCid, span_context);
